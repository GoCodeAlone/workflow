...
func (r *ModuleSchemaRegistry) registerBuiltins() {
	...
	for _, stepType := range []struct {
		...
		{"step.iac_provider_apply", "IaC Provider Apply", "Two-phase hash-guard apply via IaCProvider service"},
		{"step.iac_provider_catalog", "IaC Provider Catalog", "Fetches regions and resource types from IaCProvider service"},
		{"step.iac_provider_destroy", "IaC Provider Destroy", "Destroys resources via IaCProvider service"},
		...
		{"step.iac_provider_reconcile", "IaC Provider Reconcile", "Drift → import → approximate cloud-snapshot YAML → draft PR (review-required)"},
