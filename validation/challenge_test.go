...
func TestGenerateChallenge(t *testing.T) {
	token := GenerateChallenge("secret", "abc123")
	if token == "" {
...
func TestVerifyChallenge(t *testing.T) {
	...
	hash := "deadbeef"
	token := GenerateChallenge(secret, hash)
	if !VerifyChallenge(secret, hash, token) {
...
func TestChallengeExpiry(t *testing.T) {
	secret := "test-secret"
	hash := "test-hash"
	// Generate token for 2 hours ago — should fail verification
	oldBucket := timeBucket(time.Now().Add(-2 * time.Hour))
	oldToken := computeToken(secret, hash, oldBucket)
	if VerifyChallenge(secret, hash, oldToken) {
...
func TestChallengeDeterministic(t *testing.T) {
	secret := "test-secret"
	hash := "test-hash"
	token1 := GenerateChallenge(secret, hash)
	token2 := GenerateChallenge(secret, hash)
	if token1 != token2 {
...
func TestChallengeDifferentHash(t *testing.T) {
	secret := "test-secret"
	token1 := GenerateChallenge(secret, "hash-one")
	token2 := GenerateChallenge(secret, "hash-two")
	if token1 == token2 {
