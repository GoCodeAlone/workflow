...
func (d *Daemon) Run(ctx context.Context) error {
	for {
		// Tick failures are absorbed; nextSleep() applies exponential
		// backoff after recordFailure() bumps the counter inside Tick.
		_ = d.Tick(ctx)
		delay := d.nextSleep()
		select {
		case <-ctx.Done():
...
func (d *Daemon) nextSleep() time.Duration {
	...
	// Jitter is decorrelation, not security; math/rand/v2 is fine.
	jitter := time.Duration(rand.Int64N(int64(base) / 5)) //nolint:gosec // G404: decorrelation jitter, not crypto.
	if rand.IntN(2) == 0 {                                //nolint:gosec // same as above.
		base += jitter
	} else {
		base -= jitter
	}
