// Package pipeline provides the TemplateEngine and ExprEngine used to evaluate
// {{ }} and ${ } expressions in pipeline step configs. It has no dependency on
// the module monolith: its only workflow-internal import is interfaces/.
package pipeline
...
// ConfigLookup is a function variable that returns the value for a config key.
// It must be set by the module package at init time so that the "config" template
// function can access the global config registry without importing module/.
// If nil, the "config" template function always returns an empty string.
...
func (te *TemplateEngine) templateData(pc *interfaces.PipelineContext) map[string]any {
...
func (te *TemplateEngine) resolveValue(v any, pc *interfaces.PipelineContext) (any, error) {
	switch val := v.(type) {
	case string:
