...
| `step.iac_secret_reachability` | Pre-flight gate: checks whether `secret://` refs in plan specs are reachable from the chosen exec-env; returns per-ref verdict + `all_reachable` bool (fail-safe for remote exec-envs) | platform |
