.PHONY: build build-ui build-go test bench bench-baseline lint fmt vet fix install-hooks clean
...
bench-baseline:
	go test -bench=. -benchmem -count=6 -run=^$$ -timeout=30m ./... 2>/dev/null | tee baseline-bench.txt
...
bench-compare:
	...
	go test -bench=. -benchmem -count=6 -run=^$$ -timeout=30m ./... 2>/dev/null | tee current-bench.txt
