	...
	return []*schema.ModuleSchema{
		{
			...
			Label:       "Cloud Account",
			Category:    "cloud",
			Description: "Cloud provider credentials (AWS, GCP, Azure, Kubernetes, or mock). Used by IaC modules.",
			ConfigFields: []schema.ConfigFieldDef{
				{Key: "provider", Label: "Provider", Type: schema.FieldTypeString, Required: true, Description: "Cloud provider: aws, gcp, azure, kubernetes, mock"},
				{Key: "region", Label: "Region", Type: schema.FieldTypeString, Description: "Primary region (e.g. us-east-1, us-central1, eastus)"},
				{Key: "credentials", Label: "Credentials", Type: schema.FieldTypeJSON, Description: "Credential configuration (type, keys, paths). GCP types: service_account_json, service_account_key, workload_identity, application_default. Azure types: client_credentials, managed_identity, cli, env."},
