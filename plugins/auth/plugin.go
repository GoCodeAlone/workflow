...
func (p *Plugin) ModuleSchemas() []*schema.ModuleSchema {
	return []*schema.ModuleSchema{
		...
		},
		{
			...
			ConfigFields: []schema.ConfigFieldDef{
				{Key: "providers", Label: "Providers", Type: schema.FieldTypeArray, Required: true, Description: "List of OAuth2 provider configurations (google, github, or custom)"},
			},
		...
		},
		{
			...
			ConfigFields: []schema.ConfigFieldDef{
				...
				{Key: "clients", Label: "Registered Clients", Type: schema.FieldTypeArray, Description: "List of OAuth2 clients: [{clientId, clientSecret, scopes, description, claims}]"},
				{Key: "introspect", Label: "Introspection Policy", Type: schema.FieldTypeMap, Description: "Access-control policy for POST /oauth/introspect (allowOthers, requiredScope, requiredClaim, requiredClaimVal). Default: self-only (allowOthers: false)."},
				{Key: "endpoints", Label: "Endpoint Paths", Type: schema.FieldTypeMap, Description: "Custom OAuth2 endpoint path suffixes (token, revoke, introspect, jwks). Each value must start with '/' and all four paths must be distinct. Defaults: /oauth/token, /oauth/revoke, /oauth/introspect, /oauth/jwks."},
				{Key: "trustedKeys", Label: "Trusted External Issuers", Type: schema.FieldTypeArray, Description: "List of trusted external JWT issuers for JWT-bearer grants: [{issuer, publicKeyPEM, audiences, claimMapping}]. Supports literal \\n in PEM values for Docker/Kubernetes env vars."},
			},
