...
func (c *pluginServiceClient) GetContractRegistry(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (*ContractRegistry, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(ContractRegistry)
	err := c.cc.Invoke(ctx, PluginService_GetContractRegistry_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (UnimplementedPluginServiceServer) GetContractRegistry(context.Context, *emptypb.Empty) (*ContractRegistry, error) {
	return nil, status.Error(codes.Unimplemented, "method GetContractRegistry not implemented")
}
...
func _PluginService_GetContractRegistry_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(emptypb.Empty)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(PluginServiceServer).GetContractRegistry(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: PluginService_GetContractRegistry_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(PluginServiceServer).GetContractRegistry(ctx, req.(*emptypb.Empty))
	}
	return interceptor(ctx, in, info, handler)
}
