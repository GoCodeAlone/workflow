...
func addValidatePlanFile(path string, opts *Options, report *validatePlanReport) error {
	...
	for _, rec := range ordered {
		...
		if class == validatePlanMissing && opts != nil && opts.Fix {
			pendingStubs = append(pendingStubs, validatePlanStubText(recv, qualifier))
			site.Inserted = true
			mutated = true
		}
...
// and uses unqualified names).
//
// Review round-1 finding #7: rev0 of this function always emitted
// unqualified `*IaCPlan` / `[]PlanDiagnostic`, which fails to compile
// in the typical case where the package imports the interfaces module
// and refers to the types as `*interfaces.IaCPlan` /
// `[]interfaces.PlanDiagnostic`. Detection at the import-list level
// closes that gap.
func validatePlanStubText(recv, qualifier string) string {
	planType := "*IaCPlan"
