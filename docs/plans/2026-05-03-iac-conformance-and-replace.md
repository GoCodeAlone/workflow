...
- rev4 — addresses cycle-3 findings (rev7 — grouped by severity per cycle-6 finding):
  - **Critical fixes:**
    - **T1.5 split into T1.5 (W-1) + T3.0.5 (W-3a)** — eliminates the rev3 ghost-stub anti-pattern. T1.5 (W-1) ships only the typed-error sentinel + persisted-`--plan` path (no helper-package dependency). T3.0.5 (W-3a) ships the in-process postcondition wiring as the task after T3.1, depending on `wfctlhelpers.ApplyPlan` body. No stub `wfctlhelpers.ApplyPlan` ever ships in W-1. (T3.0.5 was renamed to T3.1.5 in rev5.)
    ...
    - **W-9's ProviderPlanner interface DROPPED entirely** per cycle-3 YAGNI finding. W-9 ships only T9.3 (cross-plugin-build CI gate) + brief docs. ProviderPlanner ships when the first concrete consumer arrives. T9.1 + T9.2 removed. Field-placement intent recorded; rev5 moved field declarations to W-3a.
  - **Important fixes:**
...
- rev6 — addresses cycle-5 findings:
  ...
  - **Important fixes:**
    ...
    - **T9.1 commit boundary made explicit**: ADR + cross-plugin-build workflow ship in ONE commit (`ci(iac): cross-plugin build gate + ADR 006 for ProviderPlanner deferral`). Single PR W-9, single commit, both artifacts atomic. (rev7 — added "006" to match the T9.1 body form per cycle-6 Important.)
...
- rev7 — addresses cycle-6 findings:
  - **Critical fixes:**
    ...
    - Applied rev4 changelog grouping (Critical / Important / Minor sub-headers) — addresses the cycle-6 self-contradicting changelog claim. The 14 flat bullets sorted by severity.
  - **Important fixes:**
    - Swept all 3 `PreservedFingerprint` capitalized residuals (T1.2 Step 3 prose header, T1.2 commit message, T1.5 §3 prose). Verified by post-sweep `grep -n "PreservedFingerprint"` returning only changelog/historical references.
    - Decoupled T1.5 ComputeDrift test from the literal `"(unset)"` placeholder. Introduced unexported `unsetFingerprintPlaceholder` constant in `iac/inputsnapshot/compute_drift.go`; tests reference the constant via in-package access.
    - T9.1 commit-message form reconciled: rev6 changelog updated to include "006" matching the T9.1 body form (`ci(iac): cross-plugin build gate + ADR 006 for ProviderPlanner deferral`).
    - W-3a rebase strategy extended: names WHO performs rebase (the implementer per `feedback_worktree_agents_must_ff_before_commit.md`); names W-1 prerequisite (W-1 must merge before T3.0.4 can rebase on its tip).
    - ADR Status changed to `Accepted (autonomous-pipeline; user-ratification deferred — see plan § Open Questions § "ProviderPlanner deferral")` per cycle-6 Important Option (b). Preserves ADR-status tooling compatibility (every prior ADR uses `Status: Accepted`); honest provenance lives in the body via `Decided by:` + Context section.
    - T3.0.4 gains "Step 4 triage path" note: 3-fields-in-one-task is a deliberate cycle-5 Option-4 trade-off; if Step 4 fails, isolate via per-field test names.
  - **Minor fixes:**
...
**Step 3: Implement (rev5/rev6 — adds preservedFingerprint sentinel (unexported per rev6) + NewTolerantEnvProvider for the in-process apply postcondition contract)**
...
git commit -m "feat(iac): add inputsnapshot.Compute + Snapshot + NewTolerantEnvProvider with preservation sentinel"
```
...
func TestComputeDrift_KeyMissingInApplySnap_ReportsDrift(t *testing.T) {
    ...
    drift := ComputeDrift(planSnap, applySnap)
    // rev7 — assert behavior, not literal placeholder string. Drift exists,
...
**Rebase strategy (rev6/rev7 — addresses cycle-5 Important on multi-commit `interfaces/iac_state.go` modifications):** rev6 consolidation makes T3.0.4 the ONLY task in W-3a that modifies `interfaces/iac_state.go` (T3.4 was rev5's secondary modifier; rev6 moved its field-add into T3.0.4). If W-3a force-pushes during review (which it WILL across cycles, per the rev1-rev6 plan history), interactive rebase uses `pick` (not `squash`) for each task commit to preserve per-task TDD-verification boundaries.
...
**Step 4 triage path (rev7 — addresses cycle-6 Important on 3-fields-in-one-task):** the 3 fields ship atomically per cycle-5 Option 4 (single `interfaces/iac_state.go` modification in W-3a). Implementer adds all 3 fields + 3 tests in Steps 1-3 before running Step 4. If Step 4 fails, isolate per-field via `go test -run TestApplyResult_<FieldName>_RoundTrip` (substitute `InputDriftReport`, `InitialInputSnapshot`, or `ReplaceIDMap`). The atomic shipment is a deliberate trade-off: cleaner Git history at the cost of slightly more granular triage when something breaks.
...
**Step 4:** If the migration includes a non-trivial decision (e.g., choosing to keep DO's upsert recovery vs. canonicalizing it), record that decision as `docs/adr/NNN-do-iac-v2-migration.md` per the workflow repo's ADR convention (3-digit numbering, Nygard-3 format with `## Status` / `## Context` / `## Decision` / `## Consequences` headings; consult the existing `docs/adr/005-field-contracts.md` as a template). The next free ADR number after W-9's `006` is `007`. Cite the codemod artifact URL in the ADR Context section. Do NOT use the `decisions/<NNNN>-...md` path from the `recording-decisions` skill default — that conflicts with the workflow repo's pre-existing convention (cycle-5 finding).
...
- W-9 dropped its ProviderPlanner interface (YAGNI per cycle-3 Important 5; ADR `docs/adr/006-providerplanner-deferred-to-first-consumer.md` records the cycle-4 reasoning; user-ratification surfaces in plan § Open Questions); W-9 is now a single CI-only PR (T9.1 = cross-plugin-build gate). It can merge anytime after W-3b lands (the gate exercises whether W-3b's interface changes break AWS/GCP/Azure compilation). It is NOT on P-DO's critical path — T9.1's CI gate runs on every plugin PR irrespective of W-9's merge status.
