...
### Step 2: Copy UI Assets for Embedding

The Go binary uses `go:embed` to bundle the UI from `module/ui_dist/`:

```bash
cp -r ui/dist/ module/ui_dist/
```

This step is required before every Go build if the UI has changed.
...
rm -rf module/ui_dist && cp -r ui/dist/ module/ui_dist/
go fmt ./...
golangci-lint run
go test -race ./...
CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w" -o server ./cmd/server
...
| `ui/` | React UI source |
| `module/ui_dist/` | Built UI (generated) |
