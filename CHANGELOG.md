...
- **`wfctl infra bootstrap` — backend-agnostic state bucket dispatch** — `bootstrapStateBackend` now dispatches by `iac.state.backend` value: `spaces` creates/verifies the DO Spaces bucket via the S3-compatible API; `s3`/`gcs`/`azure` return a clear not-yet-implemented error with contribution guidance; `filesystem`/`memory`/`postgres` are no-ops. After a successful bucket bootstrap two export lines are printed: `export WFCTL_STATE_BUCKET=<name>` (generic) and `export SPACES_BUCKET=<name>` (backend-specific, no cloud-provider prefix). The resolved bucket name is written back to the on-disk config so downstream commands load state without re-setting the env var.
